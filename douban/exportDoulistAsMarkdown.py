        ...
        if contents:
            for content in contents:
                if content.name and content.string:
                    # print("content.name:{}, content.string:{}, class: {}".format(
                    #     content.name, content.string, content.get("class")))
...
def parse(url):
    ...
    # sorted_books = sorted(books, key=lambda x: x.ratingNum, reverse=True)
    sorted_books = sorted(
        books, key=lambda x: x.getCompositeRating(), reverse=True)

    # export to markdown file
    exportToMarkdown(doulistTile, doulistAbout, sorted_books)
